...
amf_params = {'mul_emb_size': 64, 'dropout': 0.5, 'epochs': 40, 'batch_size': 256, 'learning_rate': 0.01, 'propagation_factor': None, 'neg_per_pos_sample': 1.0}
xgboost_params = {'colsample_bytree': 0.8, 'n_estimators': 100, 'learning_rate': 0.05}
...
def cross_validation(drug_drug_matrix, CV_num, seed):
    ...
    print(fold_num)
    file_results.write(
        'Method' + ' ' + 'AUC' + ' ' + 'AUPR' + ' ' + 'Precision' + ' ' + 'Recall' + ' ' + 'Accuracy' + ' ' + 'F-measure' + "\n")

    for CV in range(0, CV_num):
        nn_p_file = os.path.join('result', 'cv' + str(seed) + "_"+str(CV_num) +"_" + str(CV) + "nn_predictions.p")
        tanimoto_p_file = os.path.join('result',  'cv' + str(seed) + "_" + str(CV_num) +"_" + str(CV) + "tanimoto_predictions.p")
        ...
        #  GA
        nn = drugs_nn_predictor(train_drug_drug_matrix, testPosition, validation_tuples=None, validation_target=None, name='AMF', **amf_params)
        nn.predictions_pickle_file_name = nn_p_file
        nn.fit()
        nn.predict()
        AMF_predict_matrix = numpy.matrix(nn.predicted_mat)

        results_AMF = modelEvaluation(drug_drug_matrix, AMF_predict_matrix, testPosition, 'AMF')
        auc_score, aupr_score, precision, recall, accuracy, f,method = results_AMF
        file_results.write(method + ' ' + auc_score + ' ' + aupr_score + ' ' + precision+ ' ' + recall + ' ' + accuracy + ' ' + f + "\n")
        file_results.flush()

        tanimoto = drugs_tanimoto_predictor(train_drug_drug_matrix, testPosition)
        tanimoto.predictions_pickle_file_name = tanimoto_p_file
        tanimoto.fit()
        tanimoto.predict()
        print(xgboost_params)
        xgboost = drugs_xgboost_predictor(train_drug_drug_matrix, testPosition, **xgboost_params)
        xgboost.nn_pred = nn_p_file
        xgboost.tanimoto_pred = tanimoto_p_file
        xgboost.fit()
        xgboost.predict()
        xgb_predict_matrix = numpy.matrix(xgboost.predicted_mat)
        results_xgb = modelEvaluation(drug_drug_matrix, xgb_predict_matrix, testPosition, 'XGB')
        auc_score, aupr_score, precision, recall, accuracy, f,method = results_xgb
        file_results.write(method + ' ' + auc_score + ' ' + aupr_score + ' ' + precision+ ' ' + recall + ' ' + accuracy + ' ' + f + "\n")
        file_results.flush()
        ...
        for i in range(0,len(multiple_predict_results)):
            [auc_score, aupr_score, precision, recall, accuracy, f,featurename]=multiple_predict_results[i]
            file_results.write(featurename + ' ' + auc_score+' '+aupr_score+' '+precision+' '+recall+' '+accuracy+' '+f+"\n")
            file_results.flush()

        [auc_score, aupr_score, precision, recall, accuracy, f] = ensemble_results
        file_results.write('ensemble1' + ' ' + auc_score + ' ' + aupr_score + ' ' + precision+ ' ' + recall + ' ' + accuracy + ' ' + f + "\n")
        file_results.flush()

        [auc_score, aupr_score, precision, recall, accuracy, f] = ensemble_results_cf1
        file_results.write('cf1' + ' '+ auc_score + ' ' + aupr_score + ' ' + precision + ' ' + recall + ' ' + accuracy + ' ' + f + "\n")
        file_results.flush()

        [auc_score, aupr_score, precision, recall, accuracy, f] = ensemble_results_cf2
        file_results.write('cf2' + ' ' + auc_score + ' ' + aupr_score + ' ' + precision+ ' ' + recall + ' ' + accuracy + ' ' + f + "\n")
        file_results.flush()
...
def modelEvaluation(real_matrix,predict_matrix,testPosition,featurename): #  compute cross validation results
       ...
       predicted_probability=np.array(predicted_probability).flatten()
       ...
       print('************************AUC score:%.4f, AUPR score:%.4f, recall score:%.4f, precision score:%.4f, accuracy:%.4f, f-measure:%.4f************************' %(auc_score,aupr_score,recall,precision,accuracy,f))
       auc_score, aupr_score, precision, recall, accuracy, f = ("%.4f" % auc_score), ("%.4f" % aupr_score), ("%.4f" % precision), ("%.4f" % recall), ("%.4f" % accuracy), ("%.4f" % f)
       results=[auc_score,aupr_score,precision, recall,accuracy,f,featurename]
       return results
...
def ensemble_method(drug_drug_matrix,train_drug_drug_matrix,testPosition):
    ...
    # predict_matrix=common_similarity_matrix
    results =modelEvaluation(drug_drug_matrix,predict_matrix,testPosition,'common_similarity_label') #Common neighbor similarity
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix=MethodHub.Label_Propagation(AA_similarity_matrix,train_drug_drug_matrix)
    # predict_matrix=AA_similarity_matrix
    results =modelEvaluation(drug_drug_matrix,predict_matrix,testPosition,'AA_similarity_label') #Adamic-Adar
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix=MethodHub.Label_Propagation(RA_similarity_matrix,train_drug_drug_matrix)
    # predict_matrix=RA_similarity_matrix
    results =modelEvaluation(drug_drug_matrix,predict_matrix,testPosition,'RA_similarity_label') #Resource Allocation similarity
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix=MethodHub.Label_Propagation(Katz_similarity_matrix,train_drug_drug_matrix)
    # predict_matrix=Katz_similarity_matrix
    results =modelEvaluation(drug_drug_matrix,predict_matrix,testPosition,'Katz_similarity_label') #Katz similarity
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix=MethodHub.Label_Propagation(ACT_similarity_matrix,train_drug_drug_matrix)
    # predict_matrix=ACT_similarity_matrix
    results =modelEvaluation(drug_drug_matrix,predict_matrix,testPosition,'ACT_similarity_label') #Average Commute Time similarity
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)

    predict_matrix=MethodHub.Label_Propagation(RWR_similarity_matrix,train_drug_drug_matrix)
    # predict_matrix=RWR_similarity_matrix
    results =modelEvaluation(drug_drug_matrix,predict_matrix,testPosition,'RWR_similarity_label') #The random walk with restart similarity
    multiple_result.append(results)
    multiple_matrix.append(predict_matrix)
...
def ensemble_scoring(real_matrix, multiple_matrix, testPosition, weights,cf1,cf2):
    ...
    for i in range(0, len(testPosition)):
        ...
        aa=cf1.predict_proba(np.array(vector))
        #print(aa)
...
def calculate_metric_score(real_labels,predict_score):
   ...
   predicted_score = np.zeros(len(real_labels))
   predicted_score[predict_score.flatten() > threshold] = 1
...
runtimes=5        # implement 20 runs of 5-fold cross validation for base predictors and the ensmeble model
cv=5
drug_drug_matrix = load_csv('dataset/drug_drug_matrix.csv', 'int')
file_results_str="result/result_on_our_dataset_"+str(cv)+"CV"
weights_results_str="result/weights_on_our_dataset_"+str(cv)+"CV"
for seed in range(0, runtimes):
    ...
    cross_validation(drug_drug_matrix, cv, seed)